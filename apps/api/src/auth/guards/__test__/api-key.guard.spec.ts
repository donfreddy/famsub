...
import { HeaderKey, Permission } from '../../../common/helpers';
...
describe('ApiKeyGuard', () => {
  ...
  it('should throw ForbiddenException if API key is not sent', async () => {
    await expect(apiKeyGuard.canActivate(getExecutionContext())).rejects.toBeInstanceOf(ForbiddenException);
  ...
  it('should throw ForbiddenException if wrong API key is sent', async () => {
    ...
    await expect(apiKeyGuard.canActivate(getExecutionContext(currentKey))).rejects.toBeInstanceOf(ForbiddenException);
  ...
  it('should throw ForbiddenException if API key does not have permission', async () => {
    ...
    await expect(apiKeyGuard.canActivate(getExecutionContext(serviceKey))).rejects.toBeInstanceOf(ForbiddenException);
